...
tLoja CalculaLucro(tLoja loja)
{
    ...
    for (int i = 0; i < loja.totalVendedores; i++)
    {
        totalV += GetTotalVendido(loja.vendedores[i]);
        totalS += GetSalario(loja.vendedores[i]);
        totalC += GetComissao(loja.vendedores[i]);
    }

    loja.lucro = (totalV - (totalC + totalS + loja.aluguel));
    return loja;
}
